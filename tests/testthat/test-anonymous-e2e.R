test_that("anonymous study fixture supports ADRG and cSDRG E2E", {
  ...
  expect_true("VL.ADEFF.AVAL" %in% extracted$define_valuelevel$value_list_oid)
  expect_true("WC.ADEFF.RESP" %in% extracted$define_valuelevel$where_clause_oid)
  ...
  expect_true(any(adrg_qc$check_id == "define_valuelevel_metadata" & adrg_qc$status == "pass"))
  expect_true(any(csdrg_qc$check_id == "define_valuelevel_metadata" & csdrg_qc$status == "pass"))
})
